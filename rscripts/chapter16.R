...
# 단계 1: 데이터 셋 가져오기 
interview <- read.csv("./data/Part-4/interview.csv", header = TRUE, fileEncoding = "euc-kr")
...
#단계 2: 유클리디안 거리 계산
interview_df <- interview[c(2:7)] # 가치관 ~ 자격증
interview_df

idist <- dist(interview_df) # 유클리디안 거리분석
head(idist)
idist
# 단계 3: 계층적 군집분석
hc <- hclust(idist)
