...
pc <- prcomp(subject)
# Standard deviation 표준편차 / Proportion of Variance 변동량
summary(pc)
plot(pc)
...
# 단계 1: 상관관계 분석 - 변수 간의 상관성으로 공통요인 추출
cor(subject)

# 단계 2: 요인분석 - 요인회전법 적용(Varimax 회전법이 기본)
# 단계 2-1: 주성분 분석의 가정에 의해서 2개 요인으로 분석
result <- factanal(subject, factors = 2, rotation = "varimax")
result # p-value가 0.05 보다 작으면 요인수가 부족하다는
       # 의미로 요인수를 늘려서 다시 분석해야한다.
...
en3 <- eigen(cor(drinking_water_factor_df))
...
# Q4 를 빼고 factor 3 으로 지정해서 요인 분석
result3 <- factanal(dw_df, factor = 3 , rotation = "varimax")
result3
...
# 실습: 기술 통계량 구하기 
# 단계 1: 데이터 가져오기 
product <- read.csv("./data/Part-3/product.csv", header = TRUE, fileEncoding = "euc-kr")
head(product)
